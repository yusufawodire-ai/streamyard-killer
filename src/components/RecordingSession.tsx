...
const RecordingSession = ({
  ...
  const {
    ...
    queryFn: async () => {
      let query = supabase.from('sessions').select('*').order('created_at', {
        ascending: false
      });
      if (selectedBrand) {
  ...
  const handleDeleteAll = async () => {
    ...
    const confirmed = window.confirm(`Are you sure you want to delete ALL ${sessions.length} recording sessions? This action cannot be undone.`);
    if (!confirmed) return;
    try {
  ...
  return <div className="space-y-4">
      ...
      delay: index * 0.05
          ...
          <GlassCard variant="interactive" className="p-6 cursor-pointer" onClick={() => navigate(`/session/${session.id}`)}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-4 flex-1">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    ...
                    {session.transcript_url && <span className="flex items-center gap-1">
                        <FileText className="h-4 w-4" />
                        Transcript
                      </span>}
                    {(session.final_video_url || session.raw_video_url) && <span className="flex items-center gap-1">
              ...
              <div className="flex gap-2">
                <Button size="sm" className="glass-button" onClick={e => {
              e.stopPropagation();
              navigate(`/session/${session.id}`);
                ...
                <Button size="sm" variant="destructive" onClick={e => handleDeleteSession(session.id, e)}>
